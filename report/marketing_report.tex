...
the afternoon hours as the highest-performing conditions. All effect sizes remain 
negligible, though the consistency across both methods supports the effectiveness 
of commercial advertisement exposure.

Based on these findings, advertisers are recommended to prioritize ad delivery 
on Monday and during afternoon hours, with increased frequency targeting highly 
engaged users.
\end{abstract}
...
\textit{most.ads.hour} record the day and hour of peak ad exposure for each user, 
respectively.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{figures/conversion_rate_bar.png}
...
As shown in Figure~\ref{fig:conversion_day}, Monday yields the highest conversion 
rate in the \textit{ad} group, while Thursday, Friday, and Saturday rank lower. 
Figure~\ref{fig:conversion_hour} indicates that conversion peaks during 14:00--20:00, 
with hour 16 reaching the highest rate.
...
the conversion rate and sample size heatmaps for the \textit{ad} group. Notably, 
some cells with elevated conversion rates correspond to cells with substantially 
fewer observations, suggesting that sample size varies considerably across 
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \caption{Conversion Rate Heatmap}
...
Figure~\ref{fig:outlier_boxplot} visualizes the outlier threshold across both groups on a 
log scale. To assess the presence of extreme values in \textit{total.ads}, the IQR 
method was applied with a threshold of $3 \times \text{IQR}$, yielding an upper 
bound of 96, with 24,825 observations (4.22\%) exceeding this threshold.

Outlier users exhibit substantially higher conversion rates (17.18\% for 
\textit{ad} and 11.51\% for \textit{psa}) compared to non-outlier users 
(1.91\% and 1.31\%, respectively).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{figures/total_ads_boxplot.png}
...
Frequentist tests are evaluated using p-value and effect size: rank-biserial $r$ 
for the Mann-Whitney U test, Cohen's $h$ for the two-sample proportion test, and 
...
\subsection{Randomization Check (total.ads)}
...
The two-sample proportion test reveals a statistically significant but negligible 
difference in conversion rate between the two groups ($\chi^2(1) = 54.006$, 
$p < 0.001$, Cohen's $h = 0.053$). The \textit{ad} group achieves 2.55\% compared 
to 1.79\% for \textit{psa}.

\subsection{Subgroup Analysis (ad group only)}

\subsubsection{Conversion Rate by Day}
As shown in Table~\ref{tab:day_summary}, Monday achieves the highest conversion 
rate (3.32\%), while Saturday records the lowest (2.13\%). This pattern is 
supported by the Chi-square residuals, where Monday shows the largest positive 
residual ($+13.92$) and Saturday the largest negative ($-7.45$).
...
The Chi-square test confirms a statistically significant but negligible difference 
across days ($\chi^2(6) = 412.79$, $p < 0.001$, Cramér's $V = 0.027$), with 
conversion rates ranging from 2.13\% to 3.32\%. Pairwise tests with Bonferroni 
correction indicate that Monday differs significantly from all other days 
($p \leq 0.034$), while Thursday, Friday, and Saturday show no significant 
differences among themselves ($p = 1.000$).

\subsubsection{Conversion Rate by Hour}

Table~\ref{tab:hour_summary} presents the conversion rates for the top 10 
performing hours within the \textit{ad} group. Hour 16 achieves the highest 
conversion rate (3.09\%, residual $= +6.34$), while hour 13 records the lowest 
among the top 10 (2.51\%), a difference of 0.58 percentage points.
...
The Chi-square test confirms a statistically significant but negligible difference 
across hours ($\chi^2(23) = 403.33$, $p < 0.001$, Cramér's $V = 0.0267$). 
Pairwise tests with Bonferroni correction show that hours 15, 16, 20, and 21 
differ significantly from hour 13 ($p \leq 0.034$), while hours 14, 17, 18, 
19, and 22 show no significant differences from hour 13.

\section{Bayesian Results}

\subsection{Randomization Check (total.ads: Gamma-Poisson)}

A randomization check using the Gamma-Poisson model confirms comparable ad exposure 
between groups ($P(\textit{ad} > \textit{psa}) = 97.19\%$, 95\% HDI: $[-0.0013, 0.1284]$), 
with the HDI including zero, indicating no meaningful difference between groups.
...
As shown in Table~\ref{tab:bayes_day_summary}, Monday achieves the highest 
posterior mean (3.33\%), while Saturday records the lowest (2.13\%). The HDIs of 
Monday $[3.20\%, 3.45\%]$ and Saturday $[2.03\%, 2.23\%]$ do not overlap, 
confirming that the difference is credible rather than due to sampling variability.
...
near certainty ($P \geq 0.9993$, all 95\% HDIs exclude zero). Figure~\ref{fig:posterior_day} 
shows the posterior distributions for Monday, Wednesday, and Saturday, representing 
the high, mid, and low performing days.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.46\textwidth]{figures/posterior_day.png}
...
As shown in Table~\ref{tab:bayes_hour_summary}, hour 16 achieves the highest 
posterior mean (3.09\%), while hour 13 records the lowest (2.51\%) among the top 
10 performing hours. The HDIs of hour 16 $[2.91\%, 3.27\%]$ and hour 13 
$[2.37\%, 2.65\%]$ do not overlap, confirming a credible difference.
...
certainty ($P = 1.0000$, HDI: $[0.0036, 0.0082]$), and also shows credible 
advantages over hours 18, 19, and 22 (HDI excludes zero). However, no meaningful 
difference is found between hour 16 and hours 14, 15, 17, 20, and 21 (HDI includes 
zero), suggesting these top-performing hours achieve comparable conversion rates. 
Figure~\ref{fig:posterior_hour} shows the posterior distributions for hours 13, 14, and 16.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{figures/posterior_hour.png}
...
as the highest-performing conditions, with hour 16 achieving the peak conversion 
rate. All effect sizes remain negligible, consistent with the large sample size 
...
on \textbf{Monday} and during \textbf{afternoon hours}, particularly at 
\textbf{hour 16}, where conversion rates are consistently highest.
...
\bibitem{marketing}
Vázquez, F.
\textit{Marketing A/B Testing} [Dataset].
Kaggle.
\texttt{https://www.kaggle.com/datasets/faviovaz/marketing-ab-testing}
